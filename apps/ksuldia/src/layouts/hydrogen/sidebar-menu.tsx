...
import { usePermissions } from "@/components/rbac/can";

/**
 * Filter menu by permission. Hide items the role can't access, then drop any
 * section header (no href/permission) whose following items are all hidden.
 */
function useVisibleMenu(): MenuItem[] {
  ...
  return allowed.filter((item, index) => {
    ...
    let hasChild = false;
    for (let i = index + 1; i < allowed.length; i += 1) {
      if (!allowed[i].href) break; // next header
      hasChild = true;
      break;
    }
    return hasChild;
