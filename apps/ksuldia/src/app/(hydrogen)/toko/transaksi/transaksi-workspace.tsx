...
export default function TransaksiWorkspace({ transactions, products }: TransaksiWorkspaceProps) {
  ...
  const handleProductChange = (productId: string) => {
    ...
    if (prod) {
      ...
      const price = tab === "sale" ? Number(prod.sellingPrice) : Number(prod.purchasePrice);
      setInputPrice(price);
      setInputQty(1);
    }
  };
  ...
  const addToCart = () => {
    ...
    if (tab === "sale" && currentProduct.stock < inputQty) {
      alert(`Stok tidak cukup! Sisa stok untuk "${currentProduct.name}" adalah ${currentProduct.stock}`);
      return;
    }

    // Add or merge into cart
    setCart((prev) => {
      const existingIdx = prev.findIndex((item) => item.productId === currentProduct.id);
      if (existingIdx > -1) {
        const updated = [...prev];
    ...
    <div className="rounded-md border border-gray-200 bg-white shadow-sm">
      ...
      <div className="flex items-center gap-1 border-b border-gray-200 px-3">
        ...
        <button
          type="button"
          className={`inline-flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-semibold transition ${
            tab === "sale" ? "border-teal-700 text-teal-700" : "border-transparent text-gray-500 hover:text-gray-800"
          }`}
          onClick={() => {
            setTab("sale");
            setCart([]);
        ...
        <button
          type="button"
          className={`inline-flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-semibold transition ${
            tab === "purchase" ? "border-teal-700 text-teal-700" : "border-transparent text-gray-500 hover:text-gray-800"
          }`}
          onClick={() => {
            setTab("purchase");
            setCart([]);
      ...
      {tab === "sale" || tab === "purchase" ? (
        <div className="p-6 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-8">
          {/* Basket builder */}
          <div className="space-y-6">
            <h2 className="text-lg font-bold text-gray-900 mb-2">
              Form {tab === "sale" ? "Pencatatan Penjualan Produk" : "Pembelian Barang Supplier"}
            </h2>

            <div className="bg-gray-50 p-4 rounded-lg space-y-4 border border-gray-100">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Tambah Item ke Keranjang</p>
                  ...
                  >
                    ...
                    {products.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name} ({p.code}) {tab === "sale" ? `· sisa ${p.stock}` : ""}
                      </option>
                    ))}
                  </select>
                </div>
                    ...
                    {cart.length === 0 ? (
                      ...
                      cart.map((item) => (
                        <tr key={item.productId} className="hover:bg-gray-50/50">
                          ...
                          <td className="px-4 py-3 text-right">Rp {formatNumber(item.unitPrice)}</td>
                          ...
                          <td className="px-4 py-3 text-center">
                            ...
                            >
                              <PiTrashDuotone className="h-4 w-4 mx-auto" />
                            </button>
                          </td>
                        </tr>
                      ))
          ...
          {/* Checkout Panel */}
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-6 h-fit space-y-4">
            <h3 className="font-bold text-gray-800 border-b border-gray-200 pb-2">Ringkasan Transaksi</h3>
            
            <form action={dispatchAction} className="space-y-4">
              <input type="hidden" name="type" value={tab === "sale" ? ProductTxType.SALE : ProductTxType.PURCHASE} />
              <input type="hidden" name="items" value={JSON.stringify(cart)} />

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {tab === "sale" ? "Nama Pembeli / Keterangan" : "Nama Supplier / Keterangan"}
                </label>
                <input
                  type="text"
                  name="notes"
                  ...
                  placeholder={tab === "sale" ? "Contoh: Bp. Joko, Umum" : "Contoh: Supplier Sembako Jaya"}
                  className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-teal-600"
                />
              </div>
              ...
              <div className="border-t border-gray-200 pt-4 space-y-3">
                ...
                <div className="flex justify-between text-sm text-gray-600 border-b border-gray-250 pb-2">
                ...
                <div className="flex justify-between items-center pt-2 font-bold text-gray-900">
                  ...
                  <span className="text-teal-700 text-xl font-extrabold">Rp {formatNumber(cartTotal)}</span>
                </div>
              </div>
                    ...
                    return (
                      <>
                        <tr key={tx.id} className="hover:bg-gray-50/50">
                          ...
                          <td className="px-4 py-3 font-medium text-gray-900">{tx.notes || "-"}</td>
                          <td className="px-4 py-3 text-right font-bold text-teal-850">
                            {tx.type === ProductTxType.ADJUSTMENT ? "-" : `Rp ${formatNumber(Number(tx.totalAmount))}`}
                          </td>
                          ...
                          <td className="px-4 py-3 text-center">
                            <button
                              ...
                              className="text-teal-700 hover:text-teal-900 inline-flex items-center gap-1 font-semibold text-xs"
                            >
                              {isExpanded ? (
                                <>
                                    ...
                                    >
                                      ...
                                      <div className="text-right">
                                        <p className="font-medium text-gray-900">
                                          {item.quantity} x Rp {formatNumber(Number(item.unitPrice))}
                                        </p>
                                        {tx.type !== ProductTxType.ADJUSTMENT && (
                                          <p className="text-xs font-bold text-teal-700">
                                            Subtotal: Rp {formatNumber(Number(item.totalPrice))}
                                          </p>
                                        )}
                                      </div>
                                    </div>
