  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 border-b border-gray-200 px-4 py-3">
        ...
        {title ? (
          <span className="flex-1 text-sm font-semibold text-gray-300">{title}</span>
        ) : (
...
function SkeletonRow({
  ...
  return (
    ...
    >
      <span className={`h-3 w-2/5 rounded bg-gray-200${valueClassName ? ` ${valueClassName}` : ""}`} />
      <span className={`h-3 w-1/4 rounded bg-gray-200${valueClassName ? ` ${valueClassName}` : ""}`} />
    </div>
  );
}
